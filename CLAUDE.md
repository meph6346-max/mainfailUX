...
- **하이트맵**: `/hmaps/*.json`  ← 루트 직하 단일 레벨 (ESP3D는 중첩 createdir 미지원)
- **매크로**: `/macros/*.gco`
...
처음 사용하는 폴더(`/cfg/`, `/macros/`, `/hmaps/` 등)는 ESP3D가 인식하도록
`mf_ensureCfgFolder()` 같은 idempotent createdir을 부팅 시 호출해둔다.
이미 존재해도 createdir은 안전하다 (에러 콜백 와도 무시).

**ESP3D createdir 제약**: 루트 직하 단일 레벨 디렉토리만 생성 가능.
`path=/cfg filename=height` → HEIGHT 생성 실패 (중첩 미지원).
이 때문에 하이트맵을 `/cfg/height/` 대신 `/hmaps/` (루트 직하)에 저장함.
